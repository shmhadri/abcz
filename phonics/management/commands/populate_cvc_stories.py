"""
Django management command to populate CVC Stories
Usage: python manage.py populate_cvc_stories
"""
...
class Command(BaseCommand):
    help = 'Populate database with CVC Stories'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('بدء إضافة قصص CVC...'))
        
        # حذف القصص القديمة
        old_count = CVCStory.objects.count()
        CVCStory.objects.all().delete()
        self.stdout.write(f'✅ تم حذف {old_count} قصة قديمة')
...
يغمض سام عينيه وينام 😴''',
        ...
        for story_data in stories:
            ...
            created_count += 1
            self.stdout.write(self.style.SUCCESS(f'✅ تم إنشاء القصة: {story.title}'))
